...
const Welcome=()=>{
    ...
    const handleSubmit=(e)=>{
       const {addressTo,amount,keyword,message}=formData
        ...
        e.preventDefault();
       if (!addressTo || !amount || !keyword || !message){ alert("Value can't be blank");return;}
        sendTransaction();
    }
    return(
       <div className="flex w-full justify-center items-center">
        <div className="flex mf:flex-row flex-col items-start justify-between md:p-20 pd-12 px-4">
            <div className="flex flex-1 jusify-start flex-col mf:mr-10">
                <h1 className="text-3xl sm:text-5xl text-white text-gradient p-1">Send Crypto <br/> across the world</h1>
                ...
                <div className="grid sm:grid-cols-3 grid-cols-2 w-4=full mt-10">
                    <div className={`rounded-tl-2xl ${commanStyles}`}>
                        Realiablity
                    </div>
                    <div className={commanStyles}>Security</div>
                    <div className={`rounded-tr-2xl ${commanStyles}`}>Ethereum</div>
                    <div className={`rounded-bl-2xl ${commanStyles}`}>Web3.0</div>
